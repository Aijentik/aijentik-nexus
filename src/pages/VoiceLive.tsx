...
export default function VoiceLive() {
  ...
  const conversation = useConversation({
    ...
    onError: (e: any) => toast.error(e?.message || "Voice error"),
  });
  ...
  const start = useCallback(async () => {
    ...
    try {
      ...
      const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/voice-token`;
      ...
      await supabase.from("brain_events").insert({ venue_id: venue.id, title: "Live voice session opened", reason: "Operator connected to voice agent for testing.", severity: "info" });
    } catch (e: any) {
      toast.error(e.message || "Could not start voice");
    } finally { setBusy(false); }
  }, [conversation, venue, session]);
